...
public:
  ...
  HybridJointHandle(const hardware_interface::JointStateHandle& js, double* posDes, double* velDes, double* kp, double* kd, double* ff)
    : hardware_interface::JointStateHandle(js), posDes_(posDes), velDes_(velDes), kp_(kp), kd_(kd), ff_(ff)
  {
    if (posDes_ == nullptr)
    {
