...
namespace humanoid {
    namespace {
    size_t contactIndexToLegIndex(size_t contactPointIndex) {
        // contactNames3DoF order is [l_toe, r_toe, l_heel, r_heel].
        return contactPointIndex % 2;
    }
    }  // namespace
    ...
    vector_t FootRollConstraint::getValue(scalar_t time, const vector_t& state, const vector_t& input,
        ...
        //获取最后一个关节的速度
        const long index = 5 + static_cast<long>(contactIndexToLegIndex(contactPointIndex_)) * 6;
    ...
    VectorFunctionLinearApproximation FootRollConstraint::getLinearApproximation(scalar_t time, const vector_t& state,
        ...
        approx.dfdu.middleCols<1>(17 + static_cast<long>(contactIndexToLegIndex(contactPointIndex_)) * 6).diagonal() = vector_t::Ones(1);
        return approx;
    }
}
}
